class MemberDatatable < AjaxDatatablesRails::Base
  ...
  def get_raw_records
    ...
    Member.select("id","person_id","participant_id","participants.title as title","people.firstName","people.lastName", "participants.administrative_title","participants.email","members.email","members.isAdmin").includes(:participant).references(:participant).includes(:person).references(:person).where("members.participant_id is not null and members.person_id is not null")
