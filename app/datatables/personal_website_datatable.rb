class PersonalWebsiteDatatable < AjaxDatatablesRails::Base
  ...
  def_delegators :@view, :link_to, :edit_admin_member_personal_website_path, :admin_member_personal_website_path,
  :fa_icon,:content_tag, :image_tag, :link_to_button_column
  ...
  def data
    records.map do |record|
      [
          ...
          link_to_button_column(("<i class='fa fa-pencil'></i> Edit").html_safe, edit_admin_member_personal_website_path(member,record),
                  remote: true,  class:"btn btn-info btn-xs editButton", onclick: 'editButtonPressed("Personal Website")' ),
          link_to_button_column(("<i class='fa fa-trash-o'></i> Delete").html_safe, admin_member_personal_website_path(member,record),
